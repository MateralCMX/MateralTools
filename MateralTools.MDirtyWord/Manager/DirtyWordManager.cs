...
namespace MateralTools.MDirtyWord
{
    ...
    /// </summary>
    public class StringSearch : IKeyWordManager
    {
        #region Objects
        ...
        public DirtyWordModel[] FindAll(string text)
        {
            ...
            while (index < text.Length)
            {
                ...
                foreach (string found in ptr.Results)
                    ret.Add(new DirtyWordModel(index - found.Length + 1, found));
                index++;
            }
            return (DirtyWordModel[])ret.ToArray(typeof(DirtyWordModel));
        }
        ...
        /// </summary>
        /// <param name="text">Text to search</param>
        /// <returns>First occurrence of any keyword (or DirtyWordModel.Empty if text doesn't contain any keyword)</returns>
        public DirtyWordModel FindFirst(string text)
        {
            ArrayList ret = new ArrayList();
            TreeNode ptr = _root;
            int index = 0;
            ...
            while (index < text.Length)
            {
                ...
                foreach (string found in ptr.Results)
                    return new DirtyWordModel(index - found.Length + 1, found);
                index++;
            }
            return DirtyWordModel.Empty;
        }
